...
/* ── comment ────────────────────────────────────────── */
function toolComment({ id, identifier, author, body }) {
  if (!body || !body.trim()) return { ok: false, error: "body is required" };
  if (!author || !author.trim()) return { ok: false, error: "author is required" };
  ...
  ).run(ticket.id, author.trim(), body.trim());

  const comment = db.prepare("SELECT * FROM comments WHERE id = ?").get(result.lastInsertRowid);

  return { ok: true, result: { comment }, message: `Comment added to ${ticket.identifier} by ${author}` };
}
