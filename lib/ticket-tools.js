...
/* ── update ─────────────────────────────────────────── */
function toolUpdate({ id, identifier, status, title, description, priority, assignee, labels, blocked, blocked_reason }) {
  const db = getDb();
  ...
  const sets = [];
  const params = [];
  const fields = { title, description, status, priority, assignee, blocked, blocked_reason };
