...
/* ── update ─────────────────────────────────────────── */
function toolUpdate({ id, identifier, status, title, description, priority, assignee, labels, blocked, blocked_reason, project }) {
  const db = getDb();
  ...
  const sets = [];
  const params = [];
  const fields = { title, description, status, priority, assignee, blocked, blocked_reason, project };
...
function toolComment({ id, identifier, author, body, _agentId }) {
  ...
  try {
    ...
    maybeCreateNotification(ticket.id, body.trim(), resolvedAuthor);
  } catch {}
