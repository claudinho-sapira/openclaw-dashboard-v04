...
function maybeCreateNotification(ticketId, commentBody, author) {
  ...
  const notif = { id: result.lastInsertRowid, type, ticketId, message: messages[type], agent: author, identifier: ticket?.identifier };

  // Forward to Slack
  try {
    const { forwardToSlack } = require("./slack-notify");
    forwardToSlack(notif);
  } catch {}
